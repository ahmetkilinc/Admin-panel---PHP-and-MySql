...
$dateControl = (new \DateTime((date("Y")-1)."-01-01"))->format("Y-m-d");
$dateControl2 = (new \DateTime((date("Y")-1)."-12-31"))->format("Y-m-d");
//echo $dateC . " hey there!";

$sql = "SELECT musteri_adi, musteri_firma, musteri_tel, musteri_eposta, teklif_fiyat, teklif_tarih FROM musteri WHERE teklif_tarih > '$dateControl' AND teklif_tarih <= '$dateControl2' ORDER BY id DESC";
...
<meta charset='UTF-8' name='viewport' content='width=device-width, initial-scale=1.0'>
	...
	<button class="btnPast" onclick="goPast()"> <?php echo date("Y")-2; ?></button>
	<button class="btnNow" onclick="goNow()"><?php echo date("Y"); ?></button>
	<h2>Tüm Teklifler <?php echo date("Y")-1;?></h2>
	...
	<head>
		<script>
			...
			function goPast(){
				...
				window.location.replace('http://ahmetkilinc.net/adminPanel/tumTekliflerPast2.php');
			}
